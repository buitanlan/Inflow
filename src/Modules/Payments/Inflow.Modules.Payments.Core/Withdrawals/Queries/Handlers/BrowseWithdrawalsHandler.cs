...
internal sealed class BrowseWithdrawalsHandler(PaymentsDbContext dbContext) : IQueryHandler<BrowseWithdrawals, Paged<WithdrawalDto>>
{
    public Task<Paged<WithdrawalDto>> HandleAsync(BrowseWithdrawals query, CancellationToken cancellationToken = default)
    {
        var withdrawals = dbContext.Withdrawals.AsQueryable();

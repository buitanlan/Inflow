...
internal sealed class BrowseDepositsHandler(PaymentsDbContext dbContext) : IQueryHandler<BrowseDeposits, Paged<DepositDto>>
{
    public Task<Paged<DepositDto>> HandleAsync(BrowseDeposits query, CancellationToken cancellationToken = default)
    {
        var deposits = dbContext.Deposits.AsQueryable();
